    ...
    tabs.open({
        ...
        onReady: (tab) => {
            ...
            if (tab.url.indexOf(REDIRECT_URI + "?code=") == 0) {
                tab.attach({
                    contentScript: "document.getElementById('bitly-callback').innerHTML = " +
                                   "'Bitly OAuth<br>Addon authorized, please wait ...';"
                });
            ...
            if (tab.url.indexOf(REDIRECT_URI + "?done=true") == 0) {
                tab.attach({
                    contentScript: "document.getElementById('bitly-callback').innerHTML = " +
                                   "'Bitly OAuth<br>SUCCESS! Your access token has been " +
                                   "saved in the Easyshare preferences. Now you can " +
...
function getAccessToken(vcode) {
    let req = request({
        ...
        onComplete: (response) => {
            ...
            if (match) {
                // If it matches, the token is stored in preferences
                ...
                tabs.activeTab.url = REDIRECT_URI + "?done=true";

            } 
            else {
                // Damn! Something goes wrong...
                let rt = JSON.parse(response.text),
                    error = "<br>Error " + rt.status_code + " " + rt.status_txt;

                // A script is attached to display the message of failure.
                tabs.activeTab.attach({
                    contentScript: "document.getElementById('bitly-callback').innerHTML = " +
                                   "'Bitly OAuth<br><span class=\"fail\">FAILURE! An error " +
                                   "occurred while receiving the access token." + error + "</span>';"
                });
            }
        }
    });
