        ...
        onComplete: (credentials) => {
            ...
            if (!data.url || !data.username || !data.password) {
                notifications.notify({
                    title: _("warningTitle"),
                    text: _("warningRequiredPrefs"),
                ...
                diaspora.postMessage(data, (response) => {
                    switch(response) {
                        case 201:
                            notifications.notify({
                                title: _("successTitle"),
                                text: _("successPostSent"),
                        ...
                        default:
                            notifications.notify({
                              title: _("errorTitle"),
                              text: _("errorUnexpectedError", response),
                              iconURL: ICON_ERROR
                            });
                    }
...
function onShortenMessage(lURL) {
    let token = preferences.prefs.bitlyOAuthAccessToken;

    if (!token) { // notification warning if the addon is not authorized
        notifications.notify({
            title: _("warningTitle"),
            text: _("warningBitlyUnauthorized"),
            iconURL: ICON_WARNING
        });
    }
    else {
