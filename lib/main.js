...
/* Open a dialog window to load the Diaspora publisher and share the 
   post received by the panel */
function onPublishMessage(data) {
    ...
    if (!podURL) { // show a warning if the url of pod was not saved in prefs
        ...
        let params = "content=" + data.post + "&v=1&noui=1",
            ...
            left = (data.screenWidth-width)/2,
            top = (data.screenHeight-height)/2;
...
/* Shorten a long URL and pass the return value to the panel */
